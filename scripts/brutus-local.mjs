...
async function main() {
  ...
  if (!fileArg) {
    console.error(
      "Usage: pnpm brutus:edit <relative/path/to/file> \"instruction for Brutus\"",
    );
    process.exit(1);
  }
  ...
  if (!message || typeof message !== "string") {
    console.error(
      "Brutus did not return content. Check API keys, models, or logs.",
    );
    process.exit(1);
  }
  ...
  console.log(
    `[Brutus] Updated ${filePath} (${original.length} -> ${updated.length} chars).`,
  );
}

main().catch((error) => {
  console.error("[Brutus CLI] Error:", error);
  process.exit(1);
});
