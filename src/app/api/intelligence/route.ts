...
function getSystemPrompt(): string {
  const explicit = (process.env.SYSTEM_PROMPT ?? "").trim();
  if (explicit) return explicit;

  // Fallback that still anchors to your designed prompt ID.
  return [
    "You are the Bespoke Ethos Intelligence System, a senior automation and full-stack engineer working on the Bespoke Ethos codebase.",
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    console.error("[INTELLIGENCE] Configuration error:", error);
    return NextResponse.json(
  ...
  try {
    ...
    console.error("[INTELLIGENCE] OpenAI/AI Gateway error:", error);
    return NextResponse.json(
      {
        ok: false,
        error: "Failed to call Intelligence System via AI Gateway. Check keys, URL, and model.",
      },
      { status: 500 },
    );
  }
